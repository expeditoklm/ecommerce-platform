...
<main class="main-content-wrapper">
                            ...
                            <table class="table table-centered table-hover text-nowrap table-borderless mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr id="order-{{ $order->id }}">
                                        ...
                                        <td>
    <a href="{{ route('admin.order-single', ['uuid' => $order->uuid]) }}"
       class="fw-bold text-primary">
        {{ $order->order_number }}
                                        ...
                                        <td>
                                            <div>
                                                <span class="fw-bold">
                                                    {{ $order->user->firstname ?? '' }} {{ $order->user->name ?? 'N/A' }}
                                                </span>
                                                <br>
                                        ...
                                        <td>
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <div class="text-center">
                                                    <small class="text-muted d-block">Veut</small>
                                                    @if($order->product)
                                                    <a href="{{ route('shop.single', ['uuid' => $order->product->uuid]) }}"
                                                ...
                                                {{-- Produit proposé --}}
                                                <div class="text-center">
                                                    <small class="text-muted d-block">Propose</small>
    ...
    {{-- Tous les statuts sauf "cancelled" et le statut actuel --}}
    @foreach(\App\Models\Order::STATUSES as $statusKey => $statusData)
        @if($statusKey !== $order->status && $statusKey !== 'cancelled')
        <li>
            <a class="dropdown-item" href="#"
               onclick="updateStatus('{{ $order->uuid }}', '{{ $statusKey }}')">
                <span class="badge {{ $statusData['badge'] }} me-2">
                    {{ $statusData['label'] }}
                </span>
            </a>
        </li>
